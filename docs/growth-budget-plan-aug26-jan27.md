...
**August actuals to date (as of Sun Aug 16, 2026 — day 4 of 19):**

| Metric | Actual to date | Aug target (re-baselined) | Required run-rate | Actual run-rate | Read |
|---|---|---|---|---|---|
| Orders | **5** | 67 | 3.5/day | 1.25/day | ~24 orders projected at this rate — **43 short**. Gap is real; fix is Week 1 tasks 4/6 + ads Aug 20, not a target change. |
| Installs | **unknown** — not yet pulled from Play Console / App Store Connect | 245 | 12.9/day | — | Blocks every derived metric below. Pull both numbers today. |
| GMV | **unknown** — not yet totalled | ₹34,000 | ₹1,790/day | — | At bear AOV ₹500, 5 orders ≈ ₹2,500. Confirm from Razorpay. |
| Install→order | **not computable** (installs unknown) | 20% (bear) | — | — | This is the number that decides whether Aug 20 ads go live (D2). |
| Attribution | **none** — no promo codes existed when these 5 orders landed | — | — | — | All 5 are unattributed. Codes go live today (Week 1 task 1). |

**What the 5 orders prove and don't prove.** They prove the funnel is not dead: app → cart → 400050 address → Razorpay → delivery completes end-to-end for at least the happy path. That retires the "nobody *could* order" hypothesis that dominated the Aug 15 diagnosis. They do **not** prove the funnel converts at a healthy rate — without installs there is no install→order percentage, and D2 is a *rate* rule, not an existence rule. Channels that produced them: WhatsApp groups, Adi's own building, and friends & family — i.e. hand-to-hand personal network, none of it repeatable or measurable yet. Targets in this table are **held unchanged**; per A2's standing rule, bear-case assumptions are not revised off 4 days and 5 unattributed orders in either direction.
...
**STATUS UPDATE — Sun Aug 16, 2026 (day 4 of 19).** Two things changed since the Aug 15 entry:

1. **First orders landed: 5 orders total** (up from 0). Channels: **WhatsApp groups, Adi's own building, and friends & family** — all hand-delivered personal-network reach, no paid or systematic channel involved. The funnel produces completed orders end-to-end.
2. **iOS was live from launch day, not Aug 15.** Both stores shipped Aug 13. The Aug 15 note recording "App Store is LIVE" was the day Adi *confirmed* it, and was mis-recorded here as the go-live date. Corrected everywhere in this document.

**Revised diagnosis.** The Aug 15 entry ranked three suspects for the zero-order start. Both leading suspects are now closed:

| # | Suspect | Status as of Aug 16 |
|---|---|---|
| 1 | iOS not live | ~~RULED OUT~~ — and more decisively than thought: iOS was never missing. It was live from Aug 13, so it cannot explain any part of the slow start. |
| 2 | Funnel break (pincode gate, delivery hours, Razorpay, ops Telegram) | **RULED OUT for the happy path** — 5 orders completed end-to-end through 400050 addresses and Razorpay. Still run the task-0 test order on a friend's phone (both OSes) to catch *partial* breaks: an address the gate wrongly rejects, or a delivery-hours window that silently closes ordering for part of the day. A funnel that works for 5 people can still be rejecting a fraction of attempts. |
| 3 | No in-market demand yet | **This is now the live question**, and the answer so far is "some demand, from people who know Adi." 5 orders in 4 days is a real signal that the product works; it is not yet evidence that *strangers* buy. Nothing in this plan is proven until an order arrives from someone Adi has never met — which is exactly what societies (task 4), flyers (task 6) and ads (Aug 20) exist to test. |

**The actual problem, restated.** It is no longer "does it work" — it's **rate and attribution**. 5 orders in 4 days is 1.25/day against the 3.5/day the re-baselined August target needs (A4). And because promo codes still don't exist, none of the 5 can be traced to a source, so they teach nothing about which channel to scale. Fixing attribution (task 1) and getting the install numbers (task 0) are worth more this week than any new outreach.
...
**D2 gate — partially cleared (Aug 16).** The gate had one blocking condition: real orders must exist before money goes into traffic. **5 orders exist, so that condition is MET.** But D2 is a *rate* rule (install→first order <15%), not an existence rule, and the rate is still uncomputable because install counts haven't been pulled. So the gate stands at:

- ✅ **Orders exist end-to-end** — checkout, payment and delivery all work.
- ⏳ **Install→order rate unknown** — pull Play Console + App Store Connect installs for Aug 13–19 and compute it on **Wed Aug 19**. 5 orders on 25 installs (20%) is a healthy funnel worth feeding; 5 orders on 300 installs (1.7%) is a leak that ₹1,500/day would pour straight through.
- ⏳ **No attribution** — codes must be live before ads start, or Meta's spend can't be separated from organic.

**Decision rule for Thu Aug 20:** launch ads if install→order ≥15% **and** promo codes are live. If install→order is below 15%, hold ads and spend Week 2 fixing the drop-off instead — that is D2 firing exactly as intended, not a delay. If installs still aren't known by Aug 19, treat that as "hold" too: don't spend into a number you refused to look up.
...
### Week 1 (Aug 13–19) — LAUNCH (slipped from Aug 1–7). Spend: ~₹30,000. Goal: 80 installs, 20 orders, tracking live. **Progress at Sun Aug 16 (day 4 of 7): 5 orders of 20 · installs not yet counted.** Sequence below is re-ordered for the catch-up: diagnose first, because tasks 1–2 were skipped and the funnel is only half-verified.

**Week 1 goal check.** 5 orders in 4 days = 1.25/day; the goal needs 2.9/day. To still hit 20 by Wed Aug 19 you need 15 orders in 3 days — that will not come from the personal network, which is now largely spent. It comes from tasks 4 and 6 (societies and flyers) or it doesn't come at all. Missing 20 is survivable; **not doing tasks 1, 2, 4 and 6 a second week running is not** — that's what turns a slow start into a blind one.

0. **Sun Aug 16 — diagnose before doing anything else** (was Sat Aug 15; still not done). (a) Play Console → Statistics → Installs for Aug 13–16, **and** App Store Connect → Analytics for the same window: write down **two separate numbers**. Everything downstream needs them. (b) ✅ RESOLVED — App Store is **LIVE, and was live from launch day Aug 13**, so iOS never blocked anyone. (c) Full test order on a **friend's** phone in Bandra West, not your own (your account and address are likely whitelisted) — do it on an **iPhone and an Android**, one each. Walk them through it live: app opens to shelf? scores render? cart total appears? **does 400050 get accepted at address entry?** are delivery hours shown? does Razorpay's UPI sheet complete? does the order fire into the ops Telegram group? **Still worth doing even though 5 orders succeeded** — you're now hunting a partial break (an address format the gate rejects, a time window that silently blocks ordering), not a total one. Whatever breaks, Rohit fixes Monday. (d) One-to-one (not broadcast) message five people you know are in Bandra West: *"Did you get a chance to open the app? Genuinely — did it work, or did something break? Tell me the truth, it helps more than a compliment."* Five honest answers beat every metric on this list. (e) **New — call all 5 customers who already ordered.** They are the only real evidence you own: how did they hear, what nearly stopped them, would they order again this week. Ten minutes each, and it doubles as the B2 feedback touch.
...
### Week 2 (Aug 20–26, slipped from Aug 8–14) — Meta ads go live. Spend: ~₹27,000. Goal: 100 installs cumulative this week, 25 orders, CPI baseline measured. **Gate (updated Aug 16): the "real orders exist" half is already MET — 5 orders as of Aug 16.** The remaining condition is the rate: on **Wed Aug 19** compute install→first order from Play Console + App Store Connect installs. **≥15% and promo codes live → ads launch Aug 21 as scheduled. <15%, or installs still uncounted → hold ads and spend Week 2 on the drop-off instead.** Holding here is D2 working, not slipping further.
...
4. **Sat Aug 29 (was Mon Aug 24) — make the associate offer.** ₹20k/month, Sep 1 start, 1-month notice period, 3-month review with a path to ₹25k if they own the B5a column cleanly. Backup candidate kept warm until day 3 of the hire actually showing up.
...
| Orders to date (Aug 13–16) | **5** — unattributed (no promo codes existed); revenue not yet totalled |
...
- **Aug 16, 2026 update:** first revenue — 5 orders received Aug 13–16 from WhatsApp groups, Adi's building, and friends & family. No spend has been incurred against them (all organic/personal-network), so the ledger below is unchanged and actual paid spend remains ₹0. Order GMV is **not** budget and is not tracked here; it belongs in the metrics sheet (Week 1 task 2) and rolls into the A4 actuals table. Also corrected: iOS was live from launch day Aug 13, not Aug 15 — no ledger impact, but it removes the assumed Android-only window from every August figure.
